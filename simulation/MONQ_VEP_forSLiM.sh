...
run_vep () {
    ...
    vep \
        ...
        --output_file "${OUTDIR}/${PREFIX}.vep.sift" \
        #--vcf \
        --force_overwrite \
        --fasta "${GENOME_FASTA}" \
        --gff "${SORTED_GFF}" \
...
filter_vep()
{
    /usr/bin/singularity exec /apps/biocontainers/images/ensemblorg_ensembl-vep:release_108.2.sif filter_vep "$@"
}

for sift in deleterious tolerated
do
	filter_vep -i monq_SNP.vep.sift \
	-o monq_SNP.${sift}.txt \
	-filter "SIFT is ${sift}"
done